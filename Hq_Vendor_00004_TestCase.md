# Hq_Vendor_00004 — 검수관리 단위 테스트케이스 (v2)

> **대상 화면**: [HQ] 매입발주 > 매입관리 > 검수관리 (`hq_vendor_00004`)  
> **API Base URL**: `POST /backoffice/data/hq/vendor/hq_vendor_00004`  
...
| :--- | :--- | :--- | :--- |
| `chainNo` | **필수** | `C001` (HMS F&B 체인) | 권한별 조회 필터 및 마스터 매핑의 기준 (Mapper 바인딩) |
...
| 2 | `/getCheckDetailList` | POST | 발주 내역 상세 조회 | `List<Hq_Vendor_00004_CheckDetailListDto>` | `OBSLPHTB`, `OBSLPDTB`, `TGOODSTB`, `MNAMEMTB` |
...
sequenceDiagram
    ...
    participant Ctrl as Hq_Vendor_00004_Controller
...
*   **발생 위치**: `Hq_Vendor_00004_Sql.xml` (`saveCheckDetail`)
...
| :--- | :--- | :--- | :--- | :--- | :---: |
| **TC-101** | 정상 조건 조회 (날짜 범위) | `{"msNo":"NC0007","vendor":"","searchFromDate":"20240201","searchEndDate":"20240201","offset":0,"limit":10}` | `chainNo="C001"` | HTTP 200, 해당 조건의 전표 헤더 리스트 3건 반환 | `rows.length == 3` |
| **TC-102** | 매장 코드 입력 제한 초과 검증 | `{"msNo":"NC0007_EXCEED"}` | `chainNo="C001"` | JSP 화면 input에서 maxlength에 의해 12자 초과 입력 차단 | 화면 입력 차단 확인 |

### 5.2 `/saveCheck` — 검수내역 저장

| TC ID | 테스트 시나리오 | 입력 데이터 (JSON Body) | 세션 조건 | 기대 결과 | 판정 기준 |
| :--- | :--- | :--- | :--- | :--- | :---: |
| **TC-201** | 정상 검수수량 변경 저장 | `{"orderDate":"20240201","msNo":"NC0007","slipNo":"0001","ucostVatFg":"1","lineNoArr":[1],"checkQtyArr":["3"],"unitPriceArr":["22000"],"taxFgArr":["1"],"checkAmtArr":["66000"],"checkVatArr":["6600"],"checkFicAmtArr":["0"],"checkFicVatArr":["0"]}` | `chainNo="C001"` | HTTP 200, 저장 후 DB `obslpdtb` 내 `check_qty`가 3으로 정상 갱신 | DB `check_qty` = 3 |
...
본 화면의 MyBatis Mapper [Hq_Vendor_00004_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/vendor/Hq_Vendor_00004_Sql.xml) 쿼리 내 오라클 전용 문법 검사항목입니다.

- [ ] **Oracle ROWNUM 잔존 (L13)**: `ROWNUM RNUM` $\rightarrow$ PostgreSQL 표준 `LIMIT`/`OFFSET` 또는 `ROW_NUMBER()` 윈도우 함수로 치환 권장.
- [ ] **Oracle DECODE 함수 잔존 (L32, L92)**: `DECODE(HD.PROC_FG, '0', '등록', ...)` $\rightarrow$ ANSI 표준 문법인 `CASE WHEN` 구문으로 리팩토링 권장.
- [ ] **Oracle (+) 아우터 조인 잔존 (L167)**: `TG.ORD_UNIT = NM.NM_CD(+)` $\rightarrow$ ANSI 표준 문법인 `LEFT OUTER JOIN` 구문으로 리팩토링 권장.
