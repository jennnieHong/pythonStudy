...
| 8 | `/cancelSlip` | 무발주 입고 취소 (2루프 DT→HD 순) | `@RequestParam` 배열 | `String` | UPDATE | OBSLPDTB<br>OBSLPHTB  |
...
## 8. `/cancelSlip` — 무발주 입고 취소

**서비스 2루프 패턴**:
```
루프1: for i in list → cancelSlipDt(DT 취소)
루프2: for i in list → cancelSlipHd(HD 취소)
→ DT 전체 취소 후 HD 취소 (순서 보장)
```

| No | Request (form-param) | 예상값 |
|----|----------------------|-------|
| 8-1 | `orderDate[]=20260522&slipNo[]=SLP001&purchDate[]=20260522&msNo[]=MS001` | DT 취소 → HD 취소, chkFg=`"cancel"` |
| 8-2 | 2건 선택 | 2건 DT 취소 → 2건 HD 취소 |
| 8-3 | `orderDate[]` 없음 | 빈 배열 → 루프 0번 → `"cancel"` 반환 (오류 없음) |
| 8-4 | `orderDate[]=20260522&slipNo[]=SLP001` (purchDate, msNo 미전달) | purchDate/msNo=null → Mapper SQL 처리 확인 |
...
□ cancelSlip — 2루프 (DT 전체 → HD 전체): 중간 실패 시 일부 DT 취소 + HD 미취소 상태 가능 ★
...
□ @Transactional rollbackFor Exception — saveVendorOrder/cancelSlip/deleteSlip 전체 rollback 보장
```
