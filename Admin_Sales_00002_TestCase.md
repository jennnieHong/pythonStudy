# Admin_Sales_00002 — 어드민 당일매출 단위 테스트케이스

> **화면**: [ADMIN] 매출관리 > 영업리포트 > 당일매출  
> **URL Prefix**: `POST /backoffice/data/admin/sales/admin_sales_00002`  
...
| 세션 키 | 값 예시 | 비고 |
|---------|---------|------|
| (해당 없음) | - | 시스템 관리자 계정(admin2)으로 로그인 세션 유지 |
...
|---|---|---|---|---|---|---|---|
| 1 | `/searchSaleList` | 어드민 당일 매출 합계 및 목록 조회 | `@RequestBody` | `Map` (합계 리스트 + 목록 리스트) | SELECT | ✅ SELECT | STRNDTTB (매출상세)<br>STRNHDTB (매출헤더)<br>MGOODSTB (상품마스터)<br>MMEMBSTB (매장마스터)<br>STRNCHTB (현금상세) |

---

## 1. `/searchSaleList` — 어드민 당일 매출 합계 및 목록 조회

**Mapper 호출**: `searchDaySaleTotList`, `searchDaySaleList`  

| No | 세션 조건 | RequestBody (JSON) | 예상값 |
|----|----------|-------------------|-------|
| 1-1 | - | `{"searchFromDate":"20260622","searchEndDate":"20260622","msNo":"NC0007","posNo":""}` | NC0007 매장의 2026-06-22 정상 매출 4건에 대한 합계 요약 및 상세 목록을 담은 맵 리턴 |
| 1-2 | - | `{"searchFromDate":"20260622","searchEndDate":"20260622","msNo":"NC0007","posNo":"01"}` | NC0007 매장의 01번 POS 매출 정보 필터링 조회 성공 |
| 1-3 | - | `{"searchFromDate":"20260622","searchEndDate":"20260622","msNo":"NC9999","posNo":""}` | 존재하지 않는 매장 조회 시 빈 리스트 `{"daySaleTotList":[],"daySaleList":[]}` 반환 |
...
Invoke-WebRequest -Uri "http://localhost:8080/backoffice/login" `
  -Method POST -Body "username=admin2&password=0000" `
  -ContentType "application/x-www-form-urlencoded" `
  -SessionVariable session | Out-Null

$base = "http://localhost:8080/backoffice/data/admin/sales/admin_sales_00002"
