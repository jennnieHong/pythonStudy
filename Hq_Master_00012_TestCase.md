# Hq_Master_00012 — 본사 판매가 변경 현황 단위 테스트케이스

> **화면**: [HQ] 마스터관리 > 가격관리 > 판매가 변경 현황  
> **URL Prefix**: `POST /backoffice/data/hq/master/hq_master_00012`  
...
|---|---|---|---|---|---|---|
| 1 | `/searchChangeList` | 판매가 변경내역 조회 (페이징, goodsCdArr 조건 분기) | `@RequestBody` | `Map` (total+rows) | SELECT | TPRICETB |

---

## 1. `/searchChangeList` — 판매가 변경내역 조회
...
**★★★ `goodsCdArr` null 시 NPE**:
- `map.get("goodsCdArr")` → `null` → `.equals("ALL")` 호출 → **NullPointerException**

**★★★ `offset` null 시 NPE**:
- `(int)map.get("offset")` → `null` unboxing → **NullPointerException**

| No | 세션 조건 | RequestBody | 예상값 |
|----|----------|-------------|-------|
| 1-1 | chainNo=`C001` | `{"offset":0,"limit":10,"goodsCdArr":"ALL"}` | goodsCdList 미설정 → 전체 조회 (total+rows) |
| 1-2 | chainNo=`C001` | `{"offset":0,"limit":10,"goodsCdArr":"G001,G002"}` | `goodsCdArr` → `["G001","G002"]` split → IN 조건 조회 |
| 1-3 | chainNo=`C001` | `{"offset":0,"limit":10,"goodsCdArr":"   "}` (공백만) | `.trim().isEmpty()=true` → goodsCdList 미설정 → 전체 조회 |
...
$base = "http://localhost:18080/backoffice/data/hq/master/hq_master_00012"
$h = @{"Content-Type"="application/json"}

# 1-1. 전체 조회 (goodsCdArr=ALL)
...
□ goodsCdArr="ALL" → goodsCdList 미설정 → Mapper IN 조건 없이 전체 조회 확인
□ goodsCdArr="G001,G002" → split(",") → ["G001","G002"] → IN 조건 쿼리 확인
□ goodsCdArr 공백(" ") → trim().isEmpty()=true → goodsCdList 미설정 (전체 조회)
...
**소스 64번**: `map.get("goodsCdArr")` null 체크 없이 `.equals()` 직접 호출 → **goodsCdArr 미전송 시 NPE** ★★★  
**소스 75번**: `(int)map.get("offset")` — offset null → unboxing NPE ★★★  
**소스 65번**: `split(",")` → 단일 코드("G001")도 배열로 처리 ✅ 정상  
**엔드포인트 수**: 소스 기준 1개 (TC 파일명 `1개엔드포인트` ✅)
