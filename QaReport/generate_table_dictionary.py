            ...
            if filename.endswith(".md"):
                ...
                try:
                    ...
                    for tname in table_names:
                        ...
                        if tname_lower in filename.lower() or re.search(rf"\b{tname_lower}\b", content):
                            ...
                            for col in db_data[tname]["columns"]:
                                ...
                                if re.search(rf"\b{cname}\b", content):
      ...
      <div class="search-input-wrapper">
        ...
        <div style="margin-top: 8px; display: flex; flex-direction: column; gap: 6px; font-size: 12px; border-top: 1px solid var(--border); padding-top: 8px;">
          ...
          <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <span style="color: var(--text-muted); font-weight: 600;">��� ���� 조건 (��� �� ��� ����/����):</span>
  ...
  <div class="drawer-content" style="display: grid; grid-template-columns: 420px 1fr; gap: 24px; align-items: start;">
    ...
    <div style="display: flex; flex-direction: column; gap: 16px; min-width: 0;">
      ...
      <div class="detail-group">
        ...
        <div class="detail-label">설명 (Comment)</div>
    ...
    <div style="display: flex; flex-direction: column; gap: 16px; height: 100%; min-height: 0; overflow-y: auto; padding-right: 4px;">
      ...
      <div class="detail-group" style="background: rgba(30, 58, 138, 0.04); border: 1px dashed rgba(30, 58, 138, 0.2); padding: 20px; border-radius: 12px; display: flex; flex-direction: column; gap: 12px;">
        ...
        <textarea id="drawer-table-memo-txt" style="width: 100%; min-height: 100px; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 13px; resize: vertical; outline: none;" placeholder="테이블에 대한 기본 메모 입력..."></textarea>

        

        <!-- C -->

        <div class="detail-label" style="color: #c2410c; font-weight: 700; margin-top: 8px;">Create (등록) 메모</div>

        <textarea id="drawer-c-memo-txt" style="width: 100%; min-height: 80px; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 13px; resize: vertical; outline: none;" placeholder="등록(Insert) 관련 조건 및 비즈니스 로직 기록..."></textarea>
        ...
        <textarea id="drawer-u-memo-txt" style="width: 100%; min-height: 80px; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 13px; resize: vertical; outline: none;" placeholder="수정(Update) 관련 조건 및 비즈니스 로직 기록..."></textarea>
        ...
        <textarea id="drawer-d-memo-txt" style="width: 100%; min-height: 80px; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 13px; resize: vertical; outline: none;" placeholder="삭제(Delete/Delete flag) 관련 조건 및 비즈니스 로직 기록..."></textarea>
        ...
        <textarea id="drawer-r-memo-txt" style="width: 100%; min-height: 80px; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 13px; resize: vertical; outline: none;" placeholder="조회(Select) 시 주요 조인 조건 및 특이사항 기록..."></textarea>



        <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 12px;">

          <span id="drawer-save-status" style="font-size: 11px; color: var(--text-muted); align-self: center; margin-right: auto;">커서 이탈 시 자동 저장</span>

          <button id="drawer-save-btn" class="action-btn btn-primary" style="padding: 8px 18px; font-size: 13px; border-radius: 6px; cursor: pointer; background: var(--primary-accent); color: white; border: none;">저장 후 닫기</button>
...
<div class="toast" id="toast-msg">메모가 저장되었습니다.</div>
...
function renderTableList() {{
  ...
  matchedTables.forEach(tbl => {{
    ...
    if (tbl.comment) {{
      ...
      descParts.push('<span style="color:#cbd5e1;">��� ����</span>');
  ...
  function toggleStar(tname) {{
    ...
    if (tbl) {{
      ...
      showToast(tbl.starred ? "중요 ����블로 ��록되��������." : "중요 ����� ��록이 ��������������.");
  ...
  function renderSortQueueUI() {{
    ...
    const allOptions = [
      ...
      {{ id: "custom", name: "정렬 순서순" }},

      {{ id: "starred", name: "즐겨찾기순" }},
...
function selectTable(tbl, shouldScroll = true, openDrawerFlag = false) {{
  ...
  const colRows = tbl.columns.map(col => {{
    ...
    // 컬럼 ��명에 강조 ����� ���� 경우 처리 (��: SET_FG ��� ��)

    let formattedComment = escHtml(col.comment);

    if (col.column_name.toUpperCase() === "SET_FG") {{

      formattedComment = "<b>���� 구분</b> (0: ��� ����, 2: ������ ������ 분해 ��, 3: ���� 구성�� 분해 ��) " + formattedComment;

    }} else if (col.column_name.toUpperCase() === "PROC_FG") {{

      formattedComment = "<b>진행 ����</b> (0: ���/반려, 1: ��출확��, 2: ��������/최종����, 3: 본사����) " + formattedComment;
    ...
    const memoBoxHtml = `
      ...
      <div class="col-memo-box" style="margin-top: 6px; padding-top: 4px; border-top: 1px dotted var(--border); font-size: 12px; color: var(--text-muted);">
        ...
        <span id="memo-col-val-${{col.column_name}}" style="cursor: pointer; text-decoration: underline dotted;" onclick="editColMemo('${{tbl.table_name}}', '${{col.column_name}}')">${{currentColMemo || '���� ����...'}}</span>
  ...
  const screenBadges = tbl.screens && tbl.screens.length
    ...
    ? tbl.screens.map(scr => `<span class="badge screen-id" title="백오���� ���: ${{scr.name || ''}}">${{scr.id}}${{scr.name ? ` (${{scr.name}})` : ''}}</span>`).join(' ')
  ...
  const batchBadges = tbl.batches && tbl.batches.length
    ...
    ? tbl.batches.map(bat => `<span class="badge batch-id" title="배치 ����: ${{bat.name || ''}}">${{bat.id}}${{bat.name ? ` (${{bat.name}})` : ''}}</span>`).join(' ')

    : '<span style="color:var(--text-muted);font-size:12px;">��� 배치 ����</span>';
  ...
  const tableGuideBadges = allTableGuides.length
    ...
    : '<span style="color:var(--text-muted);font-size:12px;">��� 객��� ����</span>';
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; width: 100%;">
        ...
        <div style="background: #fff7ed; border: 1px solid #fdba74; border-radius: 8px; padding: 16px; display: flex; flex-direction: column; min-height: 250px; max-height: 350px;">
          ...
          <div style="font-weight: 700; color: #c2410c; font-size: 14px; border-bottom: 1px solid #fed7aa; padding-bottom: 6px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            ...
            <button onclick="openDrawer(currentTable)" style="padding: 2px 8px; font-size: 11px; background: transparent; border: 1px solid #c2410c; color: #c2410c; border-radius: 4px; cursor: pointer;">���</button>

          </div>

          <div id="c-memo-val-tab-${{tbl.table_name}}" style="font-size: 12.5px; color: #9a3412; white-space: pre-wrap; line-height: 1.5; overflow-y: auto; flex-grow: 1;">${{currentCMemo || '��록된 메모� ��������.'}}</div>
        ...
        <div style="background: #f0fdf4; border: 1px solid #86efac; border-radius: 8px; padding: 16px; display: flex; flex-direction: column; min-height: 250px; max-height: 350px;">
          ...
          <div style="font-weight: 700; color: #15803d; font-size: 14px; border-bottom: 1px solid #bbf7d0; padding-bottom: 6px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            ...
            <button onclick="openDrawer(currentTable)" style="padding: 2px 8px; font-size: 11px; background: transparent; border: 1px solid #15803d; color: #15803d; border-radius: 4px; cursor: pointer;">���</button>

          </div>

          <div id="u-memo-val-tab-${{tbl.table_name}}" style="font-size: 12.5px; color: #166534; white-space: pre-wrap; line-height: 1.5; overflow-y: auto; flex-grow: 1;">${{currentUMemo || '��록된 메모� ��������.'}}</div>
        ...
        <div style="background: #fef2f2; border: 1px solid #fca5a5; border-radius: 8px; padding: 16px; display: flex; flex-direction: column; min-height: 250px; max-height: 350px;">
          ...
          <div style="font-weight: 700; color: #b91c1c; font-size: 14px; border-bottom: 1px solid #fecaca; padding-bottom: 6px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            ...
            <button onclick="openDrawer(currentTable)" style="padding: 2px 8px; font-size: 11px; background: transparent; border: 1px solid #b91c1c; color: #b91c1c; border-radius: 4px; cursor: pointer;">���</button>

          </div>

          <div id="d-memo-val-tab-${{tbl.table_name}}" style="font-size: 12.5px; color: #991b1b; white-space: pre-wrap; line-height: 1.5; overflow-y: auto; flex-grow: 1;">${{currentDMemo || '��록된 메모� ��������.'}}</div>
        ...
        <div style="background: #eff6ff; border: 1px solid #93c5fd; border-radius: 8px; padding: 16px; display: flex; flex-direction: column; min-height: 250px; max-height: 350px;">
          ...
          <div style="font-weight: 700; color: #1d4ed8; font-size: 14px; border-bottom: 1px solid #bfdbfe; padding-bottom: 6px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            ...
            <button onclick="openDrawer(currentTable)" style="padding: 2px 8px; font-size: 11px; background: transparent; border: 1px solid #1d4ed8; color: #1d4ed8; border-radius: 4px; cursor: pointer;">���</button>

          </div>

          <div id="r-memo-val-tab-${{tbl.table_name}}" style="font-size: 12.5px; color: #1e40af; white-space: pre-wrap; line-height: 1.5; overflow-y: auto; flex-grow: 1;">${{currentRMemo || '��록된 메모� ��������.'}}</div>
    ...
    memoHtml = `
      ...
      <div class="memo-markdown-container" style="background-color: #fffbeb; border: 1px solid #fde68a; border-left: 4px solid #f59e0b; border-radius: 8px; padding: 16px 20px; font-size: 14px; line-height: 1.6; color: #451a03;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; border-bottom: 1.5px solid #fde68a; padding-bottom: 6px;">
          ...
          <span style="font-weight: 700; font-size: 14px; color: #b45309; display: flex; align-items: center; gap: 6px;">�� ����� ���� 메모 � �계도</span>

          <button class="memo-edit-btn" onclick="openDrawer(currentTable)" style="padding: 2px 8px; font-size: 11px; background: transparent; border: 1px solid #d97706; color: #d97706; border-radius: 4px; cursor: pointer;">��집하�</button>
  ...
  detailCard.innerHTML = `
      ...
      <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 24px; width: 100%;">
        ...
        <div class="title-row" style="flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 4px;">
          ...
          <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
            ...
            <button onclick="toggleStar('${{tbl.table_name}}')" id="star-btn-${{tbl.table_name}}" style="background: none; border: none; font-size: 20px; color: ${{isStarred ? '#f59e0b' : '#cbd5e1'}}; cursor: pointer; outline: none; padding: 0 4px; display: inline-flex; align-items: center;" title="중요 ����� 즐겨찾기">
            ...
            <div style="display: inline-flex; align-items: center; gap: 4px; margin-left: 8px;">
              ...
              <span style="font-size: 11px; color: var(--text-muted); font-weight: 600;">그룹 ����:</span>
              ...
              <select onchange="changeColor('${{tbl.table_name}}', this.value)" style="font-size: 11.5px; padding: 2px 6px; border-radius: 4px; border: 1px solid var(--border); outline: none; background: white; cursor: pointer;">
                ...
                <option value="none" ${{color === 'none' ? 'selected' : ''}}>����</option>
                ...
                <option value="yellow" ${{color === 'yellow' ? 'selected' : ''}} style="color:#eab308; font-weight:bold;">�� ����</option>

                <option value="green" ${{color === 'green' ? 'selected' : ''}} style="color:#22c55e; font-weight:bold;">�� 초록</option>

                <option value="blue" ${{color === 'blue' ? 'selected' : ''}} style="color:#3b82f6; font-weight:bold;">�� ����</option>
            ...
            <div style="display: inline-flex; align-items: center; gap: 4px; margin-left: 8px;">
              ...
              <input type="number" value="${{sortOrder}}" onchange="changeSortOrder('${{tbl.table_name}}', this.value)" style="width: 50px; font-size: 11.5px; padding: 2px 4px; border-radius: 4px; border: 1px solid var(--border); outline: none; text-align: center;" min="0" max="9999" title="������ 짠� ���� ������ ���� �중치 (������� ����)">

            </div>

          </div>

          <span class="comment" style="display: block; margin-top: 4px;">${{highlight(tbl.comment || '����� ��� ����', searchKeyword)}}</span>
        ...
        <div id="table-memo-container" class="memo-container-mini" style="cursor: pointer; width: 260px; height: 56px; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between;" onclick="openDrawer(currentTable)">
          ...
          <div class="memo-title" style="display: flex; justify-content: space-between; align-items: center; gap: 8px; font-weight: 700; color: #854d0e; font-size: 11px; margin-bottom: 2px;">
            ...
            <button class="memo-edit-btn" style="padding: 1px 4px; font-size: 10px;" onclick="event.stopPropagation(); openDrawer(currentTable)">���</button>

          </div>

          <div class="memo-content" id="table-memo-val-${{tbl.table_name}}" style="font-size: 11.5px; color: #78350f; max-height: 36px; overflow-y: auto; white-space: pre-wrap; line-height: 1.3;">${{currentTableMemo || '��록된 메모� ��������.'}}</div>
    ...
    <div class="detail-body">
      ...
      <div class="detail-tabs">
        ...
        <button id="tab-btn-fields" class="tab-btn ${{activeTab === 'fields' ? 'active' : ''}}" onclick="switchTab('fields')">컬럼 정보 & 인덱스</button>

        <button id="tab-btn-related" class="tab-btn ${{activeTab === 'related' ? 'active' : ''}}" onclick="switchTab('related')">연관 테이블 (${{tbl.related_tables ? tbl.related_tables.length : 0}})</button>

        <button id="tab-btn-memo" class="tab-btn ${{activeTab === 'memo' ? 'active' : ''}}" onclick="switchTab('memo')">상세 메모 & 설명</button>

        <button id="tab-btn-cudrmemos" class="tab-btn ${{activeTab === 'cudrmemos' ? 'active' : ''}}" onclick="switchTab('cudrmemos')">CUDR 상세 메모</button>
      ...
      <div id="tab-fields-content" class="tab-content" style="display: ${{activeTab === 'fields' ? 'flex' : 'none'}};">
        ...
        <div class="section-title" onclick="toggleIndexSection()" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center; user-select: none; margin-bottom: 8px; border-bottom: 1.5px solid #cbd5e1; padding-bottom: 6px;">
          ...
          <span id="index-toggle-arrow" style="font-size: 11px; color: var(--text-muted);">�� ��치기</span>
      ...
      <div id="tab-related-content" class="tab-content" style="display: ${{activeTab === 'related' ? 'flex' : 'none'}}; flex-direction: column; overflow-y: auto; gap: 16px; padding-bottom: 20px;">
        ...
        <div style="font-size: 13.5px; color: var(--text-muted); font-weight: 500; display: flex; flex-direction: column; width: 100%; gap: 12px;">
          ...
          <span>�� ����블과 ��무적/������베이���� ��괐� ����� 목록������. ����블명�� ��릕�� ���� ����� 명세� 바로 ����������.</span>
          ...
          <div style="display: flex; gap: 8px; width: 100%; align-items: center; background: #f8fafc; border: 1px solid var(--border); padding: 12px; border-radius: 8px; flex-wrap: wrap;">
            ...
            <span style="font-weight: 700; font-size: 13px; color: var(--text-main); margin-right: 4px;">��� ����� 추�:</span>
          ...
          <table class="grid-table" style="width: 100%; border-collapse: collapse; text-align: left; font-size: 13px; border: 1px solid var(--border);">
            ...
            <thead>
              ...
              <tr style="background: #f1f5f9; border-bottom: 2px solid var(--border);">
                ...
                <th style="padding: 10px 12px; font-weight: 600; width: 25%;">��� ����블명</th>

                <th style="padding: 10px 12px; font-weight: 600; width: 35%;">����� ��� (DB)</th>

                <th style="padding: 10px 12px; font-weight: 600; width: 30%;">�� ��� � 메모 (���� 갊�)</th>

                <th style="padding: 10px 12px; font-weight: 600; width: 10%; text-align: center;">����</th>
      ...
      <div id="tab-memo-content" class="tab-content" style="display: ${{activeTab === 'memo' ? 'flex' : 'none'}}; overflow-y: auto; gap: 16px; padding-bottom: 20px;">
        ...
        <div class="relation-row" style="display: flex; flex-direction: column; gap: 12px; font-size: 13px; color: var(--text-muted); background: #f8fafc; border: 1px solid var(--border); padding: 16px; border-radius: 8px; overflow: hidden; width: 100%;">
          ...
          <div style="display:flex; align-items:center; gap:8px; width: 100%; overflow: hidden;">
            ...
            <span style="flex-shrink: 0; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;">�� <b>��� ���</b>:</span>
          ...
          <div style="display:flex; align-items:center; gap:8px; width: 100%; overflow: hidden;">
            ...
            <span style="flex-shrink: 0; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;">��� <b>��� 배치</b>:</span>
        ...
        ${{memoHtml || '<div style="color: var(--text-muted); font-size: 13px; text-align: center; padding: 48px 0; border: 1px dashed var(--border); border-radius: 8px; background: white;">��록된 ���� 메모� ��������. ��� ������ "����� 메모" ����� ��릕� Markdown ������ �� ��명을 ��록하�� �� ��������.</div>'}}
      ...
      <div id="tab-guides-content" class="tab-content" style="display: ${{activeTab === 'guides' ? 'flex' : 'none'}}; overflow-y: auto; gap: 16px; padding-bottom: 20px;">
        ...
        <div style="font-size: 13.5px; color: var(--text-muted); font-weight: 500; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; width: 100%; gap: 16px;">
          ...
          <span style="display: flex; align-items: center; gap: 6px;">�� �� ����블과 매핑��거나 ������� ������ 분석 객��� / QA 리포�� 목록������.</span>
        ...
        <div id="table-guide-input-container" style="display: none; background: #f8fafc; border: 1px solid var(--border); padding: 16px; border-radius: 8px; width: 100%; gap: 12px; flex-direction: column;">
          ...
          <div style="font-weight: 700; font-size: 13px; color: var(--text-main);">�� ����� ��� 객��� 추�</div>

          <div style="display: flex; gap: 8px; width: 100%;">

            <input type="text" id="table-guide-input" class="input-field" style="flex: 1; font-size: 13px; padding: 6px 10px;" placeholder="객��� 마크���� ����명을 ���������� (��: my_guide.md)...">

            <button onclick="saveTableGuide('${{tbl.table_name}}')" style="background: var(--primary-accent); color: white; border: none; padding: 6px 16px; border-radius: 6px; font-size: 12.5px; font-weight: 700; cursor: pointer;">추�</button>
...
function editTableMemo(tname) {{
  ...
  container.innerHTML = `
    ...
    <div style="display: flex; flex-direction: column; gap: 8px; width: 100%;">
      ...
      <textarea id="table-memo-textarea" class="input-field" style="font-family: inherit; font-size: 13px; height: 80px; resize: vertical;" placeholder="����블에 �� 메모� ����������...">${{escHtml(currentMemo)}}</textarea>

      <div style="display: flex; gap: 8px; justify-content: flex-end;">

        <button onclick="saveTableMemo('${{tname}}')" class="btn btn-save" style="padding: 4px 12px; font-size: 12px; background: var(--primary-accent); color: white; border: none; border-radius: 4px; cursor: pointer;">��</button>
...
function editColMemo(tname, cname) {{
  ...
  span.parentElement.innerHTML = `
    ...
    <input type="text" id="memo-col-input-${{cname}}" class="input-field" style="display: inline-block; width: 60%; padding: 2px 6px; font-size: 12px; height: auto;" value="${{escHtml(currentMemo)}}" placeholder="메모 ���� �� ����...">

    <button onclick="saveColMemo('${{tname}}', '${{cname}}')" style="padding: 2px 8px; font-size: 11px; background: var(--primary-accent); color: white; border: none; border-radius: 4px; cursor: pointer; margin-left:4px;">��</button>
...
function editColGuide(tname, cname) {{
  ...
  span.parentElement.innerHTML = `
    ...
    <input type="text" id="guide-col-input-${{cname}}" class="input-field" style="display: inline-block; width: 80%; padding: 2px 6px; font-size: 11px; height: auto;" value="${{escHtml(currentGuide)}}" placeholder="D:\\path\\to\\report.md ���� ���� �� ����...">

    <div style="margin-top: 4px; display: flex; gap: 4px; justify-content: flex-end;">

      <button onclick="saveColGuide('${{tname}}', '${{cname}}')" style="padding: 2px 6px; font-size: 10px; background: var(--primary-accent); color: white; border: none; border-radius: 4px; cursor: pointer;">��</button>
...
function removeTableGuide(tableName, guideName) {{
  ...
  if (!confirm('�� ����블에�� ���� ��� 객��� 링크� ��거하��겠습���?')) return;
...
function openDrawer(tbl) {{
  ...
  document.getElementById('drawer-table-comment').textContent = tbl.comment || '����� ��� ����';
  ...
  if (tbl.screens && tbl.screens.length) {{
    ...
    screensDiv.innerHTML = tbl.screens.map(scr => `<span class="badge screen-id" style="margin-right: 4px; margin-bottom: 4px; display: inline-block;" title="백오���� ���: ${{scr.name || ''}}">${{scr.id}}${{scr.name ? ` (${{scr.name}})` : ''}}</span>`).join('');

  }} else {{

    screensDiv.innerHTML = '<span style="color:var(--text-muted);font-size:12px;">��� ��� ����</span>';
  ...
  if (tbl.batches && tbl.batches.length) {{
    ...
    batchesDiv.innerHTML = tbl.batches.map(bat => `<span class="badge batch-id" style="margin-right: 4px; margin-bottom: 4px; display: inline-block;" title="배치 ����: ${{bat.name || ''}}">${{bat.id}}${{bat.name ? ` (${{bat.name}})` : ''}}</span>`).join('');

  }} else {{

    batchesDiv.innerHTML = '<span style="color:var(--text-muted);font-size:12px;">��� 배치 ����</span>';
...
function saveDrawerTableMemo(tname) {{
  ...
  if (changed) {{
    ...
    if (miniMemoEl) miniMemoEl.textContent = text || '��록된 메모� ��������.';
    ...
    if (cEl) cEl.textContent = cText || '비어����';

    if (uEl) uEl.textContent = uText || '비어����';

    if (dEl) dEl.textContent = dText || '비어����';

    if (rEl) rEl.textContent = rText || '비어����';
...
function clearLocalMemos() {{
  ...
  if (confirm("브라우저 로컬 캐시에 저장된 메모와 중요도 설정을 초기화하고, 서버의 최신 table_memos.json 데이터로 되돌리시겠습니까?")) {{
