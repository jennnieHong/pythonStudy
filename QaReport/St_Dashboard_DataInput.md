# St_Dashboard 대시보드 (매장) 데이터 보정 가이드

본 문서는 개별 매장(가맹점) 대시보드 화면에 해당 가맹점의 상세한 실시간 지표 및 연간/상품별 매출 그래프를 렌더링하기 위해 데이터베이스에 모의 데이터를 수동으로 주입(INSERT/UPDATE)하는 방법과 쿼리를 설명합니다.

---

## 1. 대상 데이터베이스 테이블 및 목적

매장 대시보드는 세션에 로그인한 매장의 고유 가맹점 번호(`msNo`)를 검색 필터로 사용하여 다음 테이블을 집계합니다.

1. **`SDAILYTB` (일매출집계)**: 해당 가맹점의 당일 매출 카드 및 지난주 대비 성장률 지표 로드.
2. **`SMONTHTB` (월매출집계)**: 해당 가맹점의 1월~12월까지의 연간 매출추이 꺾은선 그래프 렌더링.
3. **`SGOODMTB` (상품월매출집계)**: 해당 가맹점의 당월 인기 상품 TOP 5 목록 및 금액/수량 정보 바인딩.
...
*(매장코드: `NC0007` (CAFE), 오늘 날짜: `YYYYMMDD`, 이번 달: `YYYYMM` 형식으로 변환하여 사용)*

### 2.1 일매출 집계 데이터 적재 (`SDAILYTB`)
```sql
-- NC0007 매장의 금일 매출 데이터 (오늘 날짜가 '20260706'인 경우 예시)
...
VALUES ('C001', '000', 'NC0007', '20260706', 3800000, 120, 3, 80000, 115, 5);

-- NC0007 매장의 지난주 대비 성장률용 데이터 (오늘-7일 날짜가 '20260629'인 경우 예시)
INSERT INTO hmsfns.SDAILYTB (CHAIN_NO, CHAIN_AREA, MS_NO, SALE_DATE, SALE_AMT, BILL_CNT, CANCEL_CNT, CANCEL_TOT, NATIVE_CNT, FOREIGN_CNT)
VALUES ('C001', '000', 'NC0007', '20260629', 3500000, 110, 2, 60000, 108, 2);
```
...
> [!IMPORTANT]
> **매장(ST) 권한 사용자 주의 사항**
> * 아래 2단계(일마감) 및 3단계(월마감) 화면은 **본사(HQ) 권한을 가진 관리자만 접근 및 실행**할 수 있습니다.
> * 매장 권한 사용자는 직접 마감 처리를 수행할 수 없으며, 본사에서 마감 처리를 진행하면 매장 대시보드에 집계 결과가 자동으로 반영되는 구조입니다.
> * 매장(가맹점)에서는 **1단계인 POS 매출 및 결제 수신(원천 데이터 발생)** 영역을 담당하게 됩니다.

```text
[1단계: POS 또는 연동 수신] 개별 매출 거래 데이터 적재 (가맹점 POS 영역)
  └─ 데이터: POS 매출이 수신되면 STRNBPTB(블루멤버스), STRNCMTB(할인) 등 개별 트랜잭션 테이블에 입력됨
        ▼
[2단계: 본사 일마감 처리] 일 매출 데이터를 SDAILYTB에 요약 적재 (본사 HQ 권한 필요)
  └─ 화면: [HQ] 시스템인터페이스 > 일마감 (hq_sysif_00003)
  └─ 연동 프로시저: SUB_SNETDT_P
  └─ 설명: 본사 관리자가 대상 영업일자를 선택하고 [마감실행]을 클릭하면, SUB_SNETDT_P 프로시저가 호출되어 
            당일의 개별 거래 내역을 모두 합산해 SDAILYTB(일매출집계) 테이블에 요약 로우(Row)를 자동 생성(INSERT)합니다.
        ▼
[3단계: 본사 월마감 처리] 월 매출 및 상품 판매 순위 데이터 생성 (본사 HQ 권한 필요)
  └─ 화면: [HQ] 시스템인터페이스 > 월마감 (hq_sysif_00005)
  └─ 연동 프로시저: P_MACOST_B 및 월마감 전용 프로시저
  └─ 설명: 본사 관리자가 대상 월을 선택하고 [월마감실행]을 클릭하면, SDAILYTB의 일일 집계 데이터를 한 달 치로 병합하여 
