# QA Report: Hq_Vendor_00014 구매품의발주 현황

**작성일**: 2026-06-12  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: 매입발주 > 매입현황 > 구매품의발주 현황 (hq_vendor_00014)  
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | [Hq_Vendor_00014_Controller.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/java/com/hyundai/backoffice/webapp/controller/hq/vendor/Hq_Vendor_00014_Controller.java) |
| Service | [Hq_Vendor_00014_Service.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-service/src/main/java/com/hyundai/backoffice/webapp/service/hq/vendor/Hq_Vendor_00014_Service.java) |
| Mapper (Interface) | [Hq_Vendor_00014_Mapper.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-persistence/src/main/java/com/hyundai/backoffice/webapp/dao/hq/vendor/Hq_Vendor_00014_Mapper.java) |
| SQL XML | [Hq_Vendor_00014_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/vendor/Hq_Vendor_00014_Sql.xml) |
| DTO | [Hq_Vendor_00014_GetPurchListDto.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-domain/src/main/java/com/hyundai/backoffice/webapp/dto/hq/vendor/Hq_Vendor_00014_GetPurchListDto.java) |
...
### 2.1 Base URL
```
POST /backoffice/data/hq/vendor/hq_vendor_00014/{endpoint}
```
...
|-----------|------|------|------------|
| `/search` | POST | 구매품의발주 현황 목록 조회 | SELECT (컨트롤러 L54) |
| `/detailSearch` | POST | 구매품의발주 현황 상세 목록 조회 | - |
...
[Browser UI] 조회 클릭
  └─ [Controller] /search (chainNo 세션 바인딩)
       └─ [Service] getList
            └─ [Mapper] getList (Hq_Vendor_00014_Sql.xml)
                 └─ [DB] SELECT (OBREQHTB, OBSLPHTB, MMEMBSTB 등 조인)
```

```
[Browser UI] 구매요청번호 클릭
  └─ [Controller] /detailSearch (purchReqNo, chainNo 세션 바인딩)
       └─ [Service] getDetailList
            └─ [Mapper] getDetailList (Hq_Vendor_00014_Sql.xml)
                 └─ [DB] SELECT (OBREQDTB, OBSLPDTB, TVNDRMTB, TGOODSTB 등 조인)
```
...
- **조회결과 그리드 (`#hq_vendor_00014_t01`)**: 매장, 구매요청번호, 구매요청일자, 발주일자, 구분, 합계금액, 구매요청자, 구매요청확정자 컬럼 ✅
- **상세조회 그리드 (`#hq_vendor_00014_t02`)**: 품목코드, 품목명, 거래처명, 구분, 구매규격, 구매단위, 단가, 구매(수량,금액), 발주(수량,금액,부가세) 합산 컬럼 ✅
...
| 본사 상세 조회 | `/detailSearch` | ✅ 구현 완료 | ✅ 하단 테이블 바인딩 | **PASS** |
...
| Mapper XML 인터페이스 매핑 | ✅ 정상 | com.hyundai.backoffice.webapp.dao.hq.vendor.Hq_Vendor_00014_Mapper |
...
1. **Oracle 아우터 조인 문법 `(+)` 다수 사용**
   - 향후 DB 벤더 교체 혹은 완벽한 표준 호환성을 위해 `Hq_Vendor_00014_Sql.xml` 쿼리를 ANSI LEFT OUTER JOIN 형태로 리팩토링 권장.
...
- **조회 결과 화면**: [hq_vendor_00014_search.png](file:///D:/hmTest/backoffice/QaReport/hq_vendor_00014_search.png)
- **상세 그리드 바인딩 화면**: [hq_vendor_00014_detail.png](file:///D:/hmTest/backoffice/QaReport/hq_vendor_00014_detail.png)
- **초기화(Reset) 완료 화면**: [hq_vendor_00014_reset.png](file:///D:/hmTest/backoffice/QaReport/hq_vendor_00014_reset.png)
