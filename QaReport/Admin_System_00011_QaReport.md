# QA Report: Admin_System_00011 웹 서비스 이벤트 로그 조회 (Admin)
**작성일**: 2026-06-12  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: 시스템관리 > 영업정보시스템 > 웹 서비스 이벤트 로그 조회 (관리자용: `admin_system_00011`)  
**테스트 환경**: http://localhost:8080 (로컬 개발 Tomcat 서버)  
**데이터베이스**: 192.168.10.206:5432/edb (EDB Postgres 개발 DB)  
**접속 ID/PW**: admin2 / 0000 (통합 관리자 권한)
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | `hyundai-backoffice-webapp/.../controller/admin/system/Admin_System_00011_Controller.java` |
| Service | `hyundai-backoffice-layer-service/.../service/admin/system/Admin_System_00011_Service.java` |
| Mapper (Interface) | `hyundai-backoffice-layer-persistence/.../dao/admin/system/Admin_System_00011_Mapper.java` |
| SQL XML | `hyundai-backoffice-webapp/.../resources/sqlmapper/system/Admin_System_00011_Sql.xml` |
| DTO | `hyundai-backoffice-layer-domain/.../dto/admin/system/Admin_System_00011_ServiceLogListDto.java` |
...
### 2.1 Base URL
```
POST /backoffice/data/admin/system/admin_system_00011
```
...
대상 화면(`admin_system_00011`)은 조회(SELECT) 화면으로 데이터베이스 내 수동 CUD 변경사항은 없으나, Spring AOP 설정(@ServiceLog)에 의하여 화면 진입 및 조회 시 자동으로 `hmsfns.SRVLOGTB`에 접근 로그가 기록됩니다.

### 4.1 Playwright E2E 실행 중 발생한 최신 로그 검증
- **검색 조건**: `USER_ID = 'admin2'`  
- **정렬 조건**: `CREATE_DTIME DESC, LOG_SEQ DESC`  
- **DB 조회 결과**:
```
LOG_SEQ   | USER_ID   | REMOTE_IP       | SERVICE_MENU                 | SERVICE_NAME                 | TYPE   | CREATE_DTIME
----------|-----------|-----------------|------------------------------|------------------------------|--------|---------------
1295687   | admin2    | 0:0:0:0:0:0:0:1 | 웹 서비스 이벤트 로그 조회  | 웹 서비스 이벤트 로그 조회  | SELECT | 20260612165926
1295686   | admin2    | 0:0:0:0:0:0:0:1 | 웹 서비스 이벤트 로그 조회  | 웹 서비스 이벤트 로그 조회  | SELECT | 20260612165923
1295685   | admin2    | 0:0:0:0:0:0:0:1 | 웹 서비스 이벤트 로그 조회  | 웹 서비스 이벤트 로그 조회  | SELECT | 20260612165919
```
- E2E 테스트 동작을 수행하는 과정에서 `admin2`가 호출한 이벤트 로그가 DB에 누락 없이 정확하게 자동 인서트 됨을 검증 완료했습니다. ✅
...
- **Admin - 기본 조회**: ![Admin - 기본 조회](file:///D:/hmTest/backoffice/QaReport/admin_system_00011_search.png)
- **Admin - ID 검색**: ![Admin - ID 검색](file:///D:/hmTest/backoffice/QaReport/admin_system_00011_search_id.png)
- **Admin - IP 검색**: ![Admin - IP 검색](file:///D:/hmTest/backoffice/QaReport/admin_system_00011_search_ip.png)
- **Admin - 초기화 완료**: ![Admin - 초기화 완료](file:///D:/hmTest/backoffice/QaReport/admin_system_00011_reset.png)
