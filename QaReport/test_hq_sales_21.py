    ...
    try:
        async with async_playwright() as p:
            ...
            if not success:
                ...
                try:
                    await page.screenshot(path=os.path.join(report_dir, 'hq_sales_00021_error.png'))
                except:
                    pass
                return

            print("Login successful. Navigating to hq_sales_00021 (본사 POS정산내역)...")
            await page.goto('http://localhost:8080/backoffice/view/main/hq/sales/hq_sales_00021')
            await page.wait_for_timeout(2000)
            
            await dismiss_passwords_modal(page)
            
            await page.wait_for_selector('#hq_sales_00021_t01', timeout=10000)
            print("본사 POS 정산내역 화면이 정상 로딩되었습니다.")
            ...
            await page.wait_for_selector('#selectMsPos_ms_select', state='visible', timeout=5000)
            await page.select_option('#selectMsPos_ms_select', value='NC0003')
            
            # Search
            print("Clicking 조회 button...")
            await page.locator('#hq_sales_00021_search_btn').click()
            
            # Wait for data table row
            print("Waiting for data table rows to render...")
            await page.wait_for_selector('#hq_sales_00021_t01 tbody tr td.table-onclick', timeout=10000)
            
            await page.wait_for_timeout(1000)
            await page.screenshot(path=os.path.join(report_dir, 'hq_sales_00021_search.png'))
            ...
            await page.locator('#hq_sales_00021_t01 tbody tr td.table-onclick').first.click(force=True)
            ...
            await page.wait_for_timeout(1000)
            await page.screenshot(path=os.path.join(report_dir, 'hq_sales_00021_detail.png'))
            ...
            try:
                ...
                data["screens"]["hq_sales_00021"] = {
                    "complete": True,
                    "memo": f"로그인 ID: {target_user} - 본사 POS 정산내역 (본사 권한으로 체인 C001, 매장 NC0003의 2026-06-02 정산 내역 및 상세 정산 항목 모달 조회 완료)"
                }
                
                with open(progress_path, "w", encoding="utf-8") as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
                print("Updated progress JSON successfully for hq_sales_00021.")
            ...
            print('[SUCCESS] hq_sales_00021 E2E test completed successfully.')
