# QA Report: Hq_Sales_00026 당일매출 (본사)
**작성일**: 2026-06-30  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: 매출분석 > 일/월/시간 > 당일매출 (hq_sales_00026)  
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | `hyundai-backoffice-webapp/.../controller/hq/sales/Hq_Sales_00026_Controller.java` |
| Service | `hyundai-backoffice-layer-service/.../service/hq/sales/Hq_Sales_00026_Service.java` |
| Mapper (Interface) | `hyundai-backoffice-layer-persistence/.../dao/hq/sales/Hq_Sales_00026_Mapper.java` |
| SQL XML | `hyundai-backoffice-webapp/.../sqlmapper/sales/Hq_Sales_00026_Sql.xml` |
| DTO | `hyundai-backoffice-layer-domain/.../dto/hq/sales/Hq_Sales_00026_DaySaleListDto.java` |
| JSP | `hyundai-backoffice-webapp/.../webapp/WEB-INF/views/backoffice/main/contents/hq/sales/hq_sales_00026/hq_sales_00026.jsp` |
...
### 2.1 Base URL
```
POST /backoffice/data/hq/sales/hq_sales_00026/{endpoint}
```
...
|-----------|------|------|------------|
| `/searchSaleList` | POST | 당일 매출 요약(합계) 및 상세 목록 동시 조회 | SELECT |
...
```
[Controller] Hq_Sales_00026_Controller.searchSaleList
  └─ [Service] Hq_Sales_00026_Service.searchDaySaleTotList / searchDaySaleList
       ├─ [Mapper] Hq_Sales_00026_Mapper.searchDaySaleTotList (당일 매출 합계 조회)
       └─ [Mapper] Hq_Sales_00026_Mapper.searchDaySaleList (당일 매출 상세 목록 조회)
            └─ DB Query 실행 (STRNDTTB 조인 STRNHDTB 및 STRNCHTB, STRNWETB 등)
```
...
- **테스트 시나리오**: 영업일자 `2026-06-22`, 매장 `NC0007` (조회 성공 - 단, 로그인 계정인 `fnbadmin`은 고양 F&B 본부 `NC0005` 권한으로 고정되어 타 지점인 `NC0007` 데이터는 권한 그룹 필터링에 의해 빈 그리드로 조회됨)
- **화면 조회 결과 (정합성 일치)**:
  - **합계 그리드 (t01)**:
  ...
  - **권한 동작 검증**: 본사 계정(`fnbadmin`)의 소속 매장(`ms_no`) 정보가 `NC0005`로 지정되어 있어, 웹 애플리케이션의 본사 매장 권한 제어 규칙에 따라 타 매장(`NC0007`)의 매출 정보가 화면에 출력되지 않는 정상적인 보안 제어 상태입니다. (DB 쿼리 상으로는 `NC0007` 매장의 `2026-06-22` 일자 매출 데이터 4건이 정상 적재되어 있음을 DB direct query를 통해 확인하였습니다.)
...
MyBatis SQL XML (`Hq_Sales_00026_Sql.xml`)에 PostgreSQL 마이그레이션 시 위험이 될 수 있는 레거시 구문이 다수 포함되어 있습니다:
...
| 검증 항목 | 상태 | 비고 |
|----------|------|------|
| `@RestController` 및 API 매핑 | ✅ 정상 | `/backoffice/data/hq/sales/hq_sales_00026` |
...
- 브라우저 화면 스크린샷:
  ![본사 당일매출 조회](./hq_sales_00026_search.png)
