# QA Report: Hq_Stock_00006 본사 재고조정 현황

**작성일**: 2026-06-08  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: [HQ] 재고관리 > 조정/폐기/실사 > 조정/실사 현황 (`hq_stock_00006`)  
**테스트 환경**: `http://localhost:8080` (로컬 개발 Tomcat 서버)  
**접속 ID/PW**: `fnbadmin` / `0000` (본부_고양 F&B - F&B 본사 관리자 권한)  
...
| 구분 | 파일 경로 |
| :--- | :--- |
| Controller | [Hq_Stock_00006_Controller.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/java/com/hyundai/backoffice/webapp/controller/hq/stock/Hq_Stock_00006_Controller.java) |
| Service | [Hq_Stock_00006_Service.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-service/src/main/java/com/hyundai/backoffice/webapp/service/hq/stock/Hq_Stock_00006_Service.java) |
| Mapper (Interface) | [Hq_Stock_00006_Mapper.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-persistence/src/main/java/com/hyundai/backoffice/webapp/dao/hq/stock/Hq_Stock_00006_Mapper.java) |
| SQL XML Mapper | [Hq_Stock_00006_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/stock/Hq_Stock_00006_Sql.xml) |

### 1.2 사전 데이터 등록 및 흐름 (조회 데이터 입력 원천 및 배치 연동 분석)
* **사전 데이터 입력 화면**: **본사관리 > 조정/폐기/실사 > 조정등록 (`hq_stock_00005`)** 또는 **매장관리 > 조정등록 (`st_stock_00001`)**
...
```text
  sequenceDiagram
      ...
      participant RegScreen as 조정등록 화면 (hq_stock_00005 / st_stock_00001)
      ...
      participant ViewScreen as 현황조회 화면 (hq_stock_00006 / st_stock_00002)
...
POST /backoffice/data/hq/stock/hq_stock_00006/{endpoint}
```
...
| :--- | :---: | :--- | :--- |
| `/search` | POST | 본사 조정 현황 마스터 리스트 조회 | `hmsfns.IMREALTB`, `hmsfns.TGOODSTB`, `hmsfns.MGOODSTB`, `hmsfns.MMEMBSTB`, `hmsfns.TMSSRCTB`, `hmsfns.MMSSRCTB` |
| `/searchDt` | POST | 조정 현황 특정 전표 상세 품목 리스트 조회 | `hmsfns.IMREALTB`, `hmsfns.TGOODSTB`, `hmsfns.MGOODSTB`, `hmsfns.MPRICETB`, `hmsfns.TPRICETB` |

---

## 3. 서비스 로직 및 연쇄 분석

본 화면은 본부 사용자가 과거 본사 혹은 매장에서 등록 및 확정한 재고 조정/실사 데이터를 전사 차원에서 통합 모니터링하는 **순수 조회용 (Read-Only) 서비스**입니다.
...
### 3.2 서비스 가공 특이사항
* `Hq_Stock_00006_Service.java`의 `selectModifyDtList` 메서드에서는 MyBatis에서 반환된 DTO 목록을 `Java 8 Stream API`로 파이프라인하여, 조회 조건 세션의 `chainHqYn` 값을 수동으로 삽입하는 후처리를 수행합니다. 
```java
return Hq_Stock_00006_Mapper.selectModifyDtList(commandMap.getMap()).stream()
                            .map(mapper -> {
...
로컬 Tomcat 인스턴스(`http://localhost:8080`) 및 EDB 개발 DB를 연동하여 Playwright 기반 자동화 E2E 검증을 진행하였습니다.

### 5.1 로그인 및 권한 스위칭
* 기존 브라우저 세션을 초기화한 뒤, 본사 F&B 권한인 **`fnbadmin` / `0000`** 계정으로 로그인을 실행했습니다.
* 로그인 성공 시 `fnbadmin` 사용자의 마지막 접속일시 알림 팝업(Bootbox accept)을 통과한 후 메인 대시보드로 리다이렉트됨을 확인했습니다.

### 5.2 화면 접근 및 라우팅
* 주소창을 통해 본사 조정/실사 현황 메뉴인 `/backoffice/view/main/hq/stock/hq_stock_00006`으로 이동하였습니다.
* 화면 상단의 **조정일자별/조정사유별(조정기준)** 및 **전체/전수실사/부분실사(조정구분)** 드롭다운과 매장선택, 바코드, 상품코드 등의 조회 조건 UI가 에러 없이 정상 로딩되었습니다.

### 5.3 데이터 조건 검색 및 요약 그리드 표출
* 조정기간 시작일을 `2025-01-01`, 종료일을 오늘로 지정하고 `조회`를 수행했습니다.
* 마스터 그리드인 `#hq_stock_00006_t01`에 해당 기간 동안 확정 처리된 **1건의 조정 전표 요약 데이터**가 정상적으로 호출되어 데이터가 바인딩되는 것을 검증했습니다.

### 5.4 상세 내역 조회 (마스터-디테일) 연동
* 상단 마스터 테이블의 첫 번째 행(조정일자: `2026/06/04`, 조정구분: `부분실사`, 건수: `1건`)을 마우스로 클릭하였습니다.
* 자바스크립트 클릭 이벤트 핸들러가 가동되며 `#input_surveySeq` 등의 hidden 파라미터가 갱신되었으며, 하단 상세 테이블인 `#hq_stock_00006_t02`에 상세 상품 코드(`T0000001`), 상품명, 매입가, 판매가 및 조정금액 등의 데이터가 실시간 비동기 연동(HTTP 200)됨을 확인하였습니다.

### 5.5 E2E 테스트 스크린샷 증적

#### 📸 [1] 마스터 조회 및 조정 현황 요약 화면 (hq_stock_00006_search.png)
![마스터 조회 화면](file:///D:/hmTest/backoffice/QaReport/hq_stock_00006_search.png)

#### 📸 [2] 첫 번째 행 클릭 후 하단 조정 상세 목록 화면 (hq_stock_00006_detail.png)
![디테일 연동 화면](file:///D:/hmTest/backoffice/QaReport/hq_stock_00006_detail.png)

---

## 6. SQL Mapper 검증 (Oracle -> PostgreSQL/EDB 마이그레이션 호환성)

MyBatis Mapper 파일([Hq_Stock_00006_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/stock/Hq_Stock_00006_Sql.xml))을 분석한 결과, 레거시 Oracle DB 전용 비표준 문법이 다수 잔존해 있어 마이그레이션 시 잠재적 결함 요소를 파악했습니다.
...
### 7.2 적용 소스 코드 변경 내역
* **JSP 수정 위치**: [hq_stock_00006.jsp](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/webapp/WEB-INF/views/backoffice/main/contents/hq/stock/hq_stock_00006/hq_stock_00006.jsp)
...
| 테스트 항목 | 판정 | 상세 내용 |
| :--- | :---: | :--- |
| **로그인 및 라우팅** | **✅ PASS** | `fnbadmin` 로그인 후 `/hq_stock_00006` 정상 렌더링 |
| **마스터 목록 조회** | **✅ PASS** | 1건의 조정 요약 기록 확인 및 그리드 바인딩 정상 |
...
### 💡 총평
`Hq_Stock_00006` 화면은 오라클 단가 조회 함수가 인라인 서브쿼리로 충실하게 대체되어 로컬 EDB 환경 하에서 아무런 500 예외 에러 없이 완전성 높게 조회가 수행됩니다. 또한, 검색 조건의 바코드 및 상품코드 입력 필드에 DB 컬럼 길이에 적합한 `maxlength` 속성을 추가 적용하여 비정상적인 길이의 데이터가 서버로 전송되거나 오류가 나는 경우를 사전 방지했습니다. 다만 DB 전환을 염두에 둔다면 MyBatis Mapper 내부의 오라클 전용 문법 리팩토링이 향후 권장됩니다.
