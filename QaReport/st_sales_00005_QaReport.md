# QA Report: St_Sales_00005 대비기간매출 (매장)
**작성일**: 2026-07-06  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: [ST] 매출분석 > 일/월/시간 > 대비기간매출 (st_sales_00005)  
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | `hyundai-backoffice-webapp/src/main/java/com/hyundai/backoffice/webapp/controller/st/sales/St_Sales_00005_Controller.java` |
| Service | `hyundai-backoffice-layer-service/src/main/java/com/hyundai/backoffice/webapp/service/st/sales/St_Sales_00005_Service.java` |
| Mapper (Interface) | `hyundai-backoffice-layer-persistence/src/main/java/com/hyundai/backoffice/webapp/dao/st/sales/St_Sales_00005_Mapper.java` |
| SQL XML | `hyundai-backoffice-webapp/src/main/resources/sqlmapper/sales/St_Sales_00005_Sql.xml` |
| DTO | `hyundai-backoffice-layer-domain/src/main/java/com/hyundai/backoffice/webapp/dto/st/sales/St_Sales_00005_ContrastSaleListDto.java` |
...
### 2.1 Base URL
```
POST /backoffice/data/st/sales/st_sales_00005/{endpoint}
```
...
|-----------|------|------|------------|----------|
| `/search` | POST | 매장 대비기간 매출 비교 데이터 조회 | SELECT | **단순 SELECT** (CUD 미발생) |
...
```mermaid
graph TD
    subgraph Select_Flow [st_sales_00005 조회 흐름]
...
* **테스트 계정**: `H1240961C` (비밀번호: `0000`, NC0007 매장)
...
|----|-----------------|-----------------------------|--------------------------|
| 1 | **화면 로그인** | 로그인 페이지 접속 및 매장 계정 로그인 완료 | **정상 (PASS)** |
| 2 | **화면 진입** | 메뉴 이동 및 테이블 로딩 확인 | **정상 (PASS)** |
| 3 | **조회기간/대비기간 입력** | 조회기간(2026-06-22 ~ 2026-06-23) 및 대비기간(2026-04-01 ~ 2026-04-01) 입력 | **정상 (PASS)** |
| 4 | **데이터 조회** | 로그인한 매장인 `CAFE` 단일 행만 정상 조회됨을 확인 (매출: 13,083,091원 / 대비매출: 10,000원 / 성장률: 1307.31% 일치 검증) | **정상 (PASS)** |
...
* **발견된 결함**: 성장률(`UP_RATE`)을 계산하는 공식 `A.SALE_AMT/B.C_SALE_AMT`가 포함되어 있습니다. 만약 대비기간의 순매출액(`B.C_SALE_AMT`)이 0 또는 NULL인 경우, EDB PostgreSQL 환경에서 `division by zero` 예외를 발생시키며 서버 에러가 발생할 가능성이 존재했습니다.
