...
async def run_e2e_test():
    async with async_playwright() as p:
        ...
        # 2. Go to st_vendor_00003
        url = "http://localhost:8080/backoffice/view/main/st/vendor/st_vendor_00003"
        ...
        ss_path = os.path.join(SCREENSHOT_DIR, "st_vendor_00003_search.png")
        ...
        ss_path = os.path.join(SCREENSHOT_DIR, "st_vendor_00003_modal_open.png")
        await page.screenshot(path=ss_path)
        print(f"Saved screenshot to {ss_path}")
        
        # 5. Select vendor in modal
        print("Selecting vendor '삼성웰스토리2' (000002)...")
        # selectpicker element is custom, we select vendor code 000002
        await page.evaluate("""
        ...
        ss_path = os.path.join(SCREENSHOT_DIR, "st_vendor_00003_modal_searched.png")
        await page.screenshot(path=ss_path)
        print(f"Saved screenshot to {ss_path}")
        
        # 7. Input Order Qty for first goods item (index 0) and check the checkbox
        ...
        ss_path = os.path.join(SCREENSHOT_DIR, "st_vendor_00003_modal_added.png")
        await page.screenshot(path=ss_path)
        print(f"Saved screenshot to {ss_path}")
        ...
        # Accept confirm dialog "발주를 등록 하시겠습니까?"
        ...
        print("Clicking Search to show new order...")
        await page.click("#st_vendor_00003_search_btn")
        await page.wait_for_timeout(2000)
        
        # Screenshot 5: Saved order in main list
        ss_path = os.path.join(SCREENSHOT_DIR, "st_vendor_00003_saved.png")
        await page.screenshot(path=ss_path)
        print(f"Saved screenshot to {ss_path}")
        ...
        if latest_hd:
            ...
            print(f"[DB STATE] New order found: order_date={order_date}, ms_no={ms_no}, slip_no={slip_no}, proc_fg={latest_hd[0]['proc_fg']}")
        else:
            print("[DB ERROR] New order was not inserted in OBSLPHTB!")
            await browser.close()
            return
            
        # 9. Select the saved order in main list and Confirm (확정)
        print("Checking the first order item in main table via bootstrapTable...")
        await page.evaluate("""
            $('#st_vendor_00003_t01').bootstrapTable('check', 0);
        """)
        await page.wait_for_timeout(1000)
        
        print("Clicking Confirm button...")
        await page.click("#st_vendor_00003_confirm_btn")
        await page.wait_for_timeout(1000)
        ...
        ss_path = os.path.join(SCREENSHOT_DIR, "st_vendor_00003_confirmed.png")
        await page.screenshot(path=ss_path)
        print(f"Saved screenshot to {ss_path}")
        ...
        # Depth 1: OBSLPDTB is updated to proc_fg = '2'
        # Depth 2: Tr_OBSLPD_T01_Service is triggered, but since cProcFg = '2' (not '4' or '5'), downstream수불 is NOT called.
        # Let's query IMTRLGTB (수불 로그) to prove that no log was generated for this keyBillNo
