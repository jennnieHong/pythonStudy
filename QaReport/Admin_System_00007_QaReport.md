# QA Report: Admin_System_00007 웹 메뉴 관리 (Admin)
**작성일**: 2026-06-11  
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | [Admin_System_00007_Controller.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/java/com/hyundai/backoffice/webapp/controller/admin/system/Admin_System_00007_Controller.java) |
| Service | [Admin_System_00007_Service.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-service/src/main/java/com/hyundai/backoffice/webapp/service/admin/system/Admin_System_00007_Service.java) |
| Mapper (Interface) | [Admin_System_00007_Mapper.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-persistence/src/main/java/com/hyundai/backoffice/webapp/dao/admin/system/Admin_System_00007_Mapper.java) |
| SQL XML | [Admin_System_00007_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/system/Admin_System_00007_Sql.xml) |
| JSP | [admin_system_00007.jsp](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/webapp/WEB-INF/views/backoffice/main/contents/admin/system/admin_system_00007/admin_system_00007.jsp) |
| JS | [admin_system_00007.js](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/webapp/WEB-INF/views/backoffice/main/contents/admin/system/admin_system_00007/js/admin_system_00007.js) |
...
### 2.1 Base URL
```
POST /backoffice/data/admin/system/admin_system_00007/{endpoint}
```
...
- **데이터 검증**: 분류 추가 시 중복 데이터 방지를 위해 대/중분류 코드에 대해 존재 여부를 사전에 체크하는 `menuChkL`, `menuChkM` 로직이 적용되어 있습니다.
...
- 메뉴 정보 및 매핑 정보의 변경사항은 별도의 DB 연쇄 전파(Cascading) 없이 해당 트랜잭션 내에서 안전하게 종료됩니다.
...
- **ROWNUM**: 페이징 및 행 인덱싱을 위해 ROWNUM 임시 컬럼 지정을 정상적으로 처리하고 있습니다.
- **DECODE**: `DECODE(A.SYSTEM_TYPE, 'HQ', '본부','ST', '매장', '관리자')` 등 시스템 타입 문자열 치환을 위해 사용한 Oracle DECODE 함수가 EDB Pg에서 정상적으로 실행됩니다.
- **(+) 아우터 조인**: `A.MENU_LCLASS_CD = B.MAPP_LCLASS_CD(+)` 형태의 레거시 Oracle 아우터 조인 구문이 EDB SQL 파서에 의해 호환 실행됩니다.
...
### 5.1 화면 접속 및 데이터 로드
- **서버 접속**: `http://localhost:8080/backoffice` 경로로 정상 접속하여 `admin2` 계정으로 권한 로그인에 성공하였습니다.
- **화면 진입**: 시스템관리 > 영업정보시스템 > 웹 메뉴관리 화면에 진입하여 시스템 타입이 `ADMIN`인 메뉴 설정 데이터를 정상 로드하였습니다.

### 5.2 E2E 시나리오 테스트 내역 및 스크린샷

1. **초기 화면 및 관리자 메뉴 목록 조회**
   - 조회 버튼 클릭 시 관리자 권한(`SystemType="ADMIN"`)에 속한 메뉴 구성 그리드가 안정적으로 표시됨을 확인하였습니다.
   ![조회 완료](D:/hmTest/backoffice/QaReport/admin_system_00007_search.png)
...
- **원인**: 
  - `Admin_System_00007_Sql.xml` 내 `getMmenu` select 쿼리에서 `ME.VIEW_PATH`, `ME.VIEW_FILE`, `ME.MENU_SEQ` 등의 컬럼을 조회 대상(`SELECT` 절)에서 누락하여 발생한 레거시 쿼리 결함입니다.
...
- **대상 파일**:
  - **JSP**: `admin_system_00007_M01.jsp`, `admin_system_00007_M03.jsp`, `admin_system_00007_M04.jsp`, `admin_system_00007_M05.jsp`
  - **JS**: `admin_system_00007.js`
...
| 구분 | 결과 | 비고 |
|------|------|------|
| 화면 접속 및 로그인 | ✅ PASS | admin2 관리자 로그인 성공 |
| 대/중분류 조회 | ✅ PASS | SystemType="ADMIN" 필터링 및 조인 정상 수행 |
| 타입 캐스팅 검증 | ✅ PASS | Null-Safety 패치 적용으로 빈 값 전송 오류 없음 |
