...
async def run_test():
    async with async_playwright() as p:
        ...
        try:
            print('=== STARTING TEST: hq_stock_00012 (HQ Stock Move Status) ===')
            
            # Step 1: Login
            await login(page, 'shopadmin', '0000')
            
            # Step 2: Navigate to target page
            print('Navigating directly to hq_stock_00012...')
            await page.goto('http://localhost:8080/backoffice/view/main/hq/stock/hq_stock_00012')
            await page.wait_for_timeout(3000)
            await page.wait_for_selector('#hq_stock_00012_t01', timeout=10000)
            ...
            await page.locator('#hq_stock_00012_search_btn').click()
            await page.wait_for_timeout(3000)
            
            # Save screenshot of search results
            os.makedirs('d:/hmTest/backoffice/QaReport', exist_ok=True)
            screenshot_path = 'd:/hmTest/backoffice/QaReport/hq_stock_00012_search.png'
            await page.screenshot(path=screenshot_path)
            print(f'Screenshot saved to {screenshot_path}')
            
            # Step 5: Click first row slip link to view details
            no_data = page.locator('#hq_stock_00012_t01 tbody tr.no-records-found')
            rows = page.locator('#hq_stock_00012_t01 tbody tr')
            slip_links = page.locator('#hq_stock_00012_t01 tbody tr td a.text-primary')
            ...
            if await no_data.count() == 0 and await rows.count() > 0 and await slip_links.count() > 0:
                ...
                detail_screenshot_path = 'd:/hmTest/backoffice/QaReport/hq_stock_00012_detail.png'
                await page.screenshot(path=detail_screenshot_path)
                print(f'Detail screenshot saved to {detail_screenshot_path}')
            else:
                print('[WARNING] No slips found in the table in the selected date range.')
                
            print('E2E Test for hq_stock_00012 completed successfully!')
            
        except Exception as e:
            print('Error occurred during hq_stock_00012 E2E test:', e)
            await page.screenshot(path='d:/hmTest/backoffice/QaReport/hq_stock_00012_error.png')
        finally:
            await browser.close()
