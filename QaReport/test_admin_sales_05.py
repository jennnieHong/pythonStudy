    ...
    try:
        async with async_playwright() as p:
            ...
            if not success:
                ...
                try:
                    await page.screenshot(path=os.path.join(report_dir, 'admin_sales_00005_error.png'))
                except:
                    pass
                return

            print("Login successful. Navigating to admin_sales_00005 (POS정산내역)...")
            await page.goto('http://localhost:8080/backoffice/view/main/admin/sales/admin_sales_00005')
            await page.wait_for_timeout(2000)
            
            await dismiss_passwords_modal(page)
            
            await page.wait_for_selector('#admin_sales_00005_t01', timeout=10000)
            print("POS 정산내역 화면이 정상 로딩되었습니다.")
            
            # Set search date and select box
            print("Setting searchDate to '2026-06-02' via JS...")
            await page.evaluate("() => { $('#searchDate').val('2026-06-02'); }")
            
            print("Waiting for selectMsPos dropdown elements...")
            await page.wait_for_selector('#selectMsPos_chainMs_select', timeout=5000)
            await page.select_option('#selectMsPos_chainMs_select', value='C001')
            
            # Wait for store dropdown options to populate
            await page.wait_for_timeout(1000)
            await page.wait_for_selector('#selectMsPos_chainMs2_select', state='visible', timeout=5000)
            await page.select_option('#selectMsPos_chainMs2_select', value='NC0003')
            
            # Search
            print("Clicking 조회 button...")
            await page.locator('#admin_sales_00005_search_btn').click()
            
            # Wait for data table row
            print("Waiting for data table rows to render...")
            await page.wait_for_selector('#admin_sales_00005_t01 tbody tr td.table-onclick', timeout=10000)
            
            await page.wait_for_timeout(1000)
            await page.screenshot(path=os.path.join(report_dir, 'admin_sales_00005_search.png'))
            ...
            await page.locator('#admin_sales_00005_t01 tbody tr td.table-onclick').first.click(force=True)
            ...
            await page.wait_for_timeout(1000)
            await page.screenshot(path=os.path.join(report_dir, 'admin_sales_00005_detail.png'))
            ...
            try:
                ...
                data["screens"]["admin_sales_00005"] = {
                    "complete": True,
                    "memo": f"로그인 ID: {target_user} - POS 정산내역 (어드민 권한으로 체인 C001, 매장 NC0003의 2026-06-02 정산 내역 및 상세 정산 항목 모달 조회 완료)"
                }
