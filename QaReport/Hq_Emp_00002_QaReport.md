# QA Report: Hq_Emp_00002 비밀번호변경관리
**작성일**: 2026-06-22  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: [HQ] 사원관리 > 비밀번호변경관리 (hq_emp_00002)  
**테스트 환경**: 192.168.10.206 / edb (PostgreSQL 기반 개발 DB)  
**접속ID/PW**: `shopadmin` (샵 본사 관리자) / PW: `0000` (또는 `fnbadmin` / PW: `0000`)
...
| 구분 | 파일 경로 |
|------|-----------|
| **Controller** | [Hq_Emp_00002_Controller.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/java/com/hyundai/backoffice/webapp/controller/hq/employee/Hq_Emp_00002_Controller.java) |
| **Service** | [Hq_Emp_00002_Service.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-service/src/main/java/com/hyundai/backoffice/webapp/service/hq/employee/Hq_Emp_00002_Service.java) |
| **Mapper (Interface)** | [Hq_Emp_00002_Mapper.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-persistence/src/main/java/com/hyundai/backoffice/webapp/dao/hq/employee/Hq_Emp_00002_Mapper.java) |
| **SQL XML** | [Hq_Emp_00002_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/employee/Hq_Emp_00002_Sql.xml) |
...
### 2.1 Base URL
```
POST /backoffice/data/hq/emp/hq_emp_00002
```
...
#### [텍스트 기반 처리 흐름도]
```text
[사용자] ── 1. 변경 요청 (userPW, upassWd) ──> [Controller] (HQ: validation 적용)
                                                     │
                                           2. updateUsersTb 호출
                                                     ▼
...
### 3.2 로직 검토 및 중요 포인트
- **유효성 검사(Validation) 정상 설계**:
  - `Hq_Emp_00002_Controller.modifiyCode` 메서드에는 파라미터 유효성 검증을 위한 `@NotBlank` 및 `@Size(min=1, max=4000)` 어노테이션이 정상적으로 선언되어 있습니다. 빈 값 유입이나 비정상적인 버퍼 오버플로우 입력을 입구 단계에서 안전하게 차단합니다.
- **성공 판단 기준**:
  - 비밀번호 정상 변경 완료 시 비어 있는 Map(`{}`)을 반환합니다. UI 프론트엔드 단에서는 이 결과를 성공으로 감지합니다.

---

## 4. DB 트리거 → 코드베이스 연쇄 분석

`MUSERSTB` 테이블에 CUD 발생 시 동작하는 트리거 로직입니다.
...
1. **Depth 1: MUSERSTB 수정**
   - 사원 비밀번호 변경 시 `MUSERSTB` 테이블에 CUD 수정 발생.
2. **Depth 2: SSUSERTB & MMSLOGTB 연쇄 발생**
   - **POS 전송 데이터 생성**: `SSUSERTB` 테이블에 `INSERT` 발생. 사원 번호, 가맹점 번호, 사원명, POS 주문자구분 등 전송 이력을 작성합니다.
...
- **대상 파일**: [Hq_Emp_00002_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/employee/Hq_Emp_00002_Sql.xml) L35
...
> - **E2E 검증 시나리오**: `shopadmin` 계정 로그인 ➔ `hq_emp_00002` 화면 진입 ➔ 기존 비밀번호 `0000` 입력 ➔ 신규 비밀번호 `0000a123!` 입력 및 확인 ➔ 저장 클릭 ➔ Bootbox Confirm 확인 클릭 ➔ 알림 다이얼로그(`비밀번호가 변경되었습니다.`) 성공 감지 ➔ 실제 DB 해시값 및 메타데이터(`FST_LOGIN_PW_CHANGE`='N') 수정 여부 확인 완료.
...
| Controller 유효성 검사 어노테이션 | ✅ 정상 | `Hq_Emp_00002_Controller` 내 `@NotBlank`, `@Size` 정상 적용됨 |
...
### 🟡 Warning (마이그레이션 권고)
1. **Oracle 전용 함수 및 시퀀스 구문 하드코딩**:
   - SQL Mapper 파일(`Hq_Emp_00002_Sql.xml`, `Tr_MUSERS_T01_Sql.xml`)에 잔존하는 `SYSDATE` 및 `SSUSERSQ.nextval` 구문을 PostgreSQL 표준 방식(`NOW()`, `nextval('hmsfns.SSUSERSQ')`)으로 교체할 것을 마이그레이션 작업에 권고합니다.
...
| validation 정밀성 | ✅ PASS (유효성 검사 어노테이션 적용됨) |
| SQL 호환성 | ⚠️ Warning (Oracle 전용 문법 잔존) |
| 브라우저 테스트 | ✅ PASS (Playwright E2E 동적 검증 완료) |
| **종합 판정** | **⚠️ 부분 PASS (화면 E2E 검증 완료이나 SQL 마이그레이션 보완 필요)** |
