# QA Report: Hq_Commu_00001 본사 공지사항 조회

**작성일**: 2026-06-29  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: 본사 백오피스 > 매입관리 > 커뮤니케이션 > 공지사항 (hq_commu_00001)  
**테스트 환경**: http://localhost:8080 (로컬 개발 서버)  
**접속ID/PW**: `shopadmin / 0000` (본사 관리자 계정)  
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | [Hq_Commu_00001_Controller.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/java/com/hyundai/backoffice/webapp/controller/hq/communication/Hq_Commu_00001_Controller.java) |
| Service | [Hq_Commu_00001_Service.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-service/src/main/java/com/hyundai/backoffice/webapp/service/hq/communication/Hq_Commu_00001_Service.java) |
| Mapper (Interface) | [Hq_Commu_00001_Mapper.java](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-layer-persistence/src/main/java/com/hyundai/backoffice/webapp/dao/hq/communication/Hq_Commu_00001_Mapper.java) |
| SQL XML | [Hq_Commu_00001_Sql.xml](file:///d:/workspace/hmotors/workspace_hms20260326/backoffice/hyundai-backoffice-webapp/src/main/resources/sqlmapper/communication/Hq_Commu_00001_Sql.xml) |
...
### 2.1 Base URL
```
POST /backoffice/data/hq/communication/hq_commu_00001/{endpoint}
```
...
본사용 공지사항 조회는 가맹점 화면과 다르게 **사전 권한 확인(`checkNotice`) 없이 즉시 상세 열람**으로 진입합니다.

```
[Controller] noticeDetailView
  ├─ increaseNoticeHits()              → 조회수 증가 처리 (Transactional)
  │    ├─ updateHitsLog()              → BBSLOGTB MERGE INTO (조회 로그 적재)
  │    └─ updateNoticeHits()           &rarr; BBSNTCTB HITS + 1 (조회수 가산)
  └─ getNoticeDetailData()             &rarr; 공지 상세 본문 및 첨부파일 정보 로딩
...
* **분석 결과**:
  ...
  * `updateHitsLog` 쿼리는 본사의 단순 조회 횟수를 누적하기 위해 `A.HIT_COUNT = HIT_COUNT + 1` 수식을 사용하거나, 최초 적재 시 고정값 `1`을 대입합니다.
...
| 로그인 ID / PW | `shopadmin / 0000` (본사 관리자 권한) |
| 화면 경로 | 본사 백오피스 > 매입관리 > 커뮤니케이션 > 공지사항 |
...
* **상세 결과 데이터**:
  ...
  * **조회수 증가 검증**:
    * 상세 모달 오픈 전 `BBSNTCTB.HITS` 값: **`7`** (앞서 가맹점 테스트 시 누적 가산 완료된 상태)
    * 첫 번째 공지사항 상세 모달 클릭 후 `BBSNTCTB.HITS` 값: **`8`** (정상 가산 확인 ✅)
  * **조회 로그 적재 검증**:
    * 본사 계정(`shopadmin`, 소속 매장 `NC0005`)에 대해 상세 조회 실행 시 `updateHitsLog` (MERGE INTO)를 통해 `BBSLOGTB` 조회수가 가산되는 메커니즘을 DB 수준에서 확인했습니다. (정상 적재 확인 ✅)
...
현재 개발 디렉토리 상의 `Hq_Commu_00001_Sql.xml` 쿼리는 다음과 같습니다.
1. **`(+)` 아우터 조인 문법 잔존**: 
...
| 검증 항목 | 상태 | 비고 |
|----------|------|------|
| `@RestController`, `@RequestMapping` 어노테이션 | ✅ 정상 | `/backoffice/data/hq/communication/hq_commu_00001` |
| `@Transactional` 주입 및 롤백 규칙 설정 | ✅ 정상 | `rollbackFor = {RuntimeException.class, Exception.class}` |
| 상세 조회 시 조회수 가산 및 로그 적재 | ✅ 정상 | Hits 1 가산 및 로그 테이블 갱신 처리 |
...
1. **Oracle 아우터 조인 `(+)` 문법 잔존**: 
   * `Hq_Commu_00001_Sql.xml` 내 `selectNoticeList` 쿼리의 `(+)` 조인을 표준 ANSI `LEFT JOIN` 형식으로 리팩토링할 것을 권장합니다.
...
## 10. 첨부 (E2E 테스트 캡처화면)
* **공지사항 목록 화면**:
  ![본사 공지 목록](file:///D:/hmTest/backoffice/QaReport/hq_commu_00001_1_list.png)
* **공지사항 상세 조회 모달 화면**:
  ![본사 공지 상세 모달](file:///D:/hmTest/backoffice/QaReport/hq_commu_00001_2_detail_modal.png)
