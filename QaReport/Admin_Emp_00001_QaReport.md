# QA Report: Admin_Emp_00001 어드민 사원 관리
**작성일**: 2026-06-22  
**작성자**: AI QA Agent (Antigravity)  
**대상 화면**: [Admin] 사원관리 (admin_emp_00001)  
...
| 구분 | 파일 경로 |
|------|-----------|
| Controller | `hyundai-backoffice-webapp/.../controller/admin/employee/Admin_Emp_00001_Controller.java` |
| Service | `hyundai-backoffice-layer-service/.../service/admin/employee/Admin_Emp_00001_Service.java` |
| Mapper (Interface) | `hyundai-backoffice-layer-persistence/.../dao/admin/employee/Admin_Emp_00001_Mapper.java` |
| SQL XML | `hyundai-backoffice-webapp/.../sqlmapper/employee/Admin_Emp_00001_Sql.xml` |
...
### 2.1 Base URL
```
POST /backoffice/data/admin/employee/admin_emp_00001/{endpoint}
```
...
|-----------|------|------|------------|
| `/search/hq` | POST | 본사 사원 목록 조회 | SELECT |
| `/search/store` | POST | 매장 사원 목록 조회 | SELECT |
| `/searchDetail/hq` | POST | 본사 사원 상세 조회 | SELECT |
| `/searchDetail/shop` | POST | 매장 사원 상세 조회 | SELECT |
| `/idDupChk` | POST | 아이디 중복 체크 | - |
| `/userSave` | POST | 일반 사용자 등록 | INSERT |
| `/userAdminSave` | POST | Admin 사용자 등록 | INSERT |
| `/IDDelete` | POST | 사용자 삭제 (본인 PW 인증 필요) | DELETE |
| `/userUpdate` | POST | 사원 정보 수정 (본인 PW 인증 필요) | UPDATE |
| `/adminUserUpdate` | POST | Admin 사원 수정 (userRole 없음) | UPDATE |
| `/pwReset` | POST | 비밀번호 초기화 | UPDATE |
| `/moveEmp` | POST | 사원 매장 이동 | UPDATE |
| `/searchUser` | POST | 메뉴복사용 사원 조회 | SELECT |
| `/copyMenu` | POST | 사원 메뉴 복사 | UPDATE |
| `/selectRole` | POST | 권한별 메뉴 조회 | SELECT |
| `/unlock` | POST | 계정 잠금 해제 | UPDATE |

---

## 3. 서비스 로직 분석 (코드베이스 변환 검증)

### 3.1 사원 등록 및 어드민 등록 흐름 (`userSave`, `userAdminSave`)
- `userSave`는 채번 후 일반 사원을 생성하고, 권한이 있을 경우 `insertUserRole`을 연동 호출합니다.
- `userAdminSave`는 관리자 사원 전용 생성이며, `getAdminEmpiId`를 통해 채번하고 `insertAdminUser`를 호출합니다. 일반 등록과 달리 `userRole` 분기 처리가 배제되어 있습니다.
- 모든 등록 후 즉시 `tr_MUSERS_T01_Service`가 호출되어 SSUSERTB 등에 실시간 POS 동기화를 수행합니다.

### 3.2 사원 수정 및 어드민 수정 흐름 (`userUpdate`, `adminUserUpdate`)
- `adminUserUpdate` 컨트롤러 호출 시, `orderFg`를 항상 `"0"`으로 고정 하드코딩하고, `userRole` 파라미터가 누적되지 않도록 구현되어 있어 서비스 단에서 역할 변경 분기 처리가 일어나지 않도록 안전히 차단되어 있습니다.
- 모든 수정 동작 완료 시 변경값이 존재하면 `tr_MUSERS_T01_Service`에 의해 `MMSLOGTB`에 데이터 변경 로그가 실시간 기재됩니다.

### 3.3 계정 잠금 해제 흐름 (`unlock`)
- 로그인 5회 이상 실패 시 `ACCT_LOCK = 'Y'` 처리된 계정에 대해, `updateUnLock` 쿼리를 호출하여 `ACCT_LOCK = 'N'`, `AUTH_FAILURE_CNT = 0`으로 데이터 복구를 수행합니다.
...
|------|-----------|---------|---------|------|
| 본사 사원 목록 조회 | `/search/hq` | ✅ 구현 완료 | ✅ 데이터 표시 | **PASS** |
| 매장 사원 목록 조회 | `/search/store` | ✅ 구현 완료 | ✅ 데이터 표시 | **PASS** |
| 본사 사원 상세 조회 | `/searchDetail/hq` | ✅ 구현 완료 | ✅ 상세 정보 렌더링 | **PASS** |
| 매장 사원 상세 조회 | `/searchDetail/shop` | ✅ 구현 완료 | ✅ 상세 정보 렌더링 | **PASS** |
...
| 계정 잠금 해제 | `/unlock` | ✅ 구현 완료 | ✅ 해제 기능 동작 | **PASS** |
...
- `Admin_Emp_00001_Sql.xml` 내의 `selectHqEmpList`, `selectStoreEmpList`, `selectHqEmpDtList` 쿼리에 Oracle 레거시 문법인 외부조인 `(+)`가 존재합니다:
  ```sql
  AND MV.MS_NO (+)= MU.MS_NO
  AND MV.VENDOR(+)= MU.VENDOR
  AND MU.CREATE_ID = MU2.USER_ID(+)
  ```
  - EDB(EPAS) 호환성 모드에서는 오동작 없이 통과하지만, 향후 완전한 PostgreSQL 호환성을 위해 `LEFT OUTER JOIN`으로 쿼리를 ANSI 표준화할 것을 강력히 권고합니다.
...
- 어드민 사원관리 화면 최초 진입: ![admin_emp_00001_initial](/C:/Users/uoshj/.gemini/antigravity-ide/brain/1a4236fc-92c9-48e6-b3ea-a3f6617131ea/admin_emp_00001_initial.png)
- 신규 어드민사원(`playfnbadmin`) 등록 완료: ![admin_emp_00001_saved](/C:/Users/uoshj/.gemini/antigravity-ide/brain/1a4236fc-92c9-48e6-b3ea-a3f6617131ea/admin_emp_00001_saved.png)
- 어드민사원 정보 수정 완료: ![admin_emp_00001_updated](/C:/Users/uoshj/.gemini/antigravity-ide/brain/1a4236fc-92c9-48e6-b3ea-a3f6617131ea/admin_emp_00001_updated.png)
- 어드민사원 삭제 완료: ![admin_emp_00001_deleted](/C:/Users/uoshj/.gemini/antigravity-ide/brain/1a4236fc-92c9-48e6-b3ea-a3f6617131ea/admin_emp_00001_deleted.png)
