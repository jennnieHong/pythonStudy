# St_Sales_00005 — 매장 대비기간매출 단위 테스트케이스

> **화면**: [ST] 매출분석 > 일/월/시간 > 대비기간매출  
> **URL Prefix**: `POST /backoffice/data/st/sales/st_sales_00005`  
...
| 세션 키 | 값 예시 | 비고 |
|---------|---------|------|
| `chainNo` | `C001` | C001 소속 매장 로그인 세션 정보 |
| `msNo` | `NC0007` | 가맹점 로그인 세션 정보 (자동 세션 매핑) |
...
|---|---|---|---|---|---|---|---|
| 1 | `/search` | 매장 대비기간 매출 데이터 조회 | `@RequestBody` | `List<St_Sales_00005_ContrastSaleListDto>` | SELECT | ✅ SELECT | SDAILYTB (매출 일집계)<br>MMEMBSTB (매장마스터) |

---

## 1. `/search` — 매장 대비기간 매출 데이터 조회

**Mapper 호출**: `getContrastSaleList`  

| No | 세션 조건 | RequestBody (JSON) | 예상값 |
|----|----------|-------------------|-------|
| 1-1 | chainNo=`C001`, msNo=`NC0007` | `{"fromDate":"20260601","toDate":"20260630","confromDate":"20260501","contoDate":"20260531"}` | NC0007 매장의 6월 매출과 5월 대비 매출 비교 데이터 정상 로드 |
| 1-2 | chainNo=`C001`, msNo=`NC0007` | `{"fromDate":"20260630","toDate":"20260601","confromDate":"20260501","contoDate":"20260531"}` | 날짜 선후 관계 검증 (화면 스크립트 단에서 팝업으로 조회 차단) |
...
$base = "http://localhost:8080/backoffice/data/st/sales/st_sales_00005"
$h = @{"Content-Type"="application/json"}

# 1-1. 매장 대비기간 매출 조회 (2026년 6월 vs 5월)
Invoke-RestMethod -Uri "$base/search" -Method POST -Headers $h -WebSession $session `
  -Body '{"fromDate":"20260601","toDate":"20260630","confromDate":"20260501","contoDate":"20260531"}'
```
...
□ Mapper 인터페이스와 MyBatis SQL XML 연동 상태 검증 [Mapper L82] ✅
