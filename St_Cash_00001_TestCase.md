...
for(int i = 0; i < array.size(); i++) {
    list.add(addMap);
    St_Cash_00001_Mapper.deleteYnUpdate(list);  ← 매 iteration마다 누적된 list 전달 ★
}
// deleteYnUpdate(list)를 루프 안에서 호출
// i=0: list=[A001], i=1: list=[A001, A002], i=2: list=[A001, A002, A003]
// → 중복 처리 위험 (A001이 3번 삭제 시도) ★★
```
...
## 6. E2E 실측 테스트 결과 및 검증 완료 내역 (2026-06-26 실측)

Playwright E2E 브라우저 및 EDB DB 정합성 실측 결과, JS 미정의 변수 결함 해결 후 CUD 테스트케이스가 최종 **PASS** 판정되었습니다.

### 6.1 [PASS] TC 4-1: 수정 API (`/cashUpdate`) 검증 완료
* **테스트 입력**: `{"accioDate":"20260624", "accioNo":"0001", "anctAmt":"65000", "vat":"6500"}`
* **실측 동작 결과**: 
  - `st_cash_00001.js` 파일 내 전송 데이터에서 무의미한 미정의 변수들(`custCd`, `remark`, `userId`)을 삭제하는 패치를 적용하고 톰캣 배포를 재수행하였습니다.
  - 브라우저단에서 `ReferenceError` 없이 `/cashUpdate` 요청이 성공적으로 전송되었으며 모달이 정상 닫혔습니다.
* **DB 확인 결과**: `ACNT_AMT`가 `65000.00`, `VAT`가 `6500.00`으로 DB 테이블 `MACCIOTB`에 즉시 업데이트 완료됨을 실측 검증했습니다.

### 6.2 [PASS] TC 5-1: 삭제 API (`/cashDel`) 검증 완료
* **테스트 입력**: `{"accioDate":"20260624", "delAccioNoArr":["0001"]}`
* **실측 동작 결과**:
  - 정상적으로 체크박스 선택 후 [내역삭제] 버튼 동작이 호출되어 confirm 대화상자 수락 후 삭제 프로세스가 정상 진행되었습니다.
* **DB 확인 결과**: 데이터의 `DELETE_YN` 컬럼 상태값이 `'Y'`로 업데이트되어 논리 삭제 완료되었음을 실측 검증했습니다.
