...
def update_memos():
    ...
    if "njs_inter_test_use" not in tables:
        print("njs_inter_test_use key not found in tables. Initializing...")
        tables["njs_inter_test_use"] = {
            "memo": "NJS_INTER_TEST_USE 테이블 정보입니다. (포스 연동 인터페이스 테스트 결제 상세)",
            "starred": False,
            "color": "none",
            "columns": {},
    ...
    njs_inter_test_use = tables["njs_inter_test_use"]
    
    # Add/Update related_tables with all verified related tables in the telex/interface testing flow
    njs_inter_test_use["related_tables"] = [
        {
            "table_name": "njs_inter_test",
            "description": "[인터페이스 테스트 마스터] NJS_INTER_TEST_USE.TR_NO = NJS_INTER_TEST.TR_NO 조인을 통해 테스트 거래 마스터 전표 정보와 개별 결제 수단(현금, 카드, 제휴사 포인트 등) 승인 상세 내역을 N:1 관계로 대조합니다."
        },
        {
            "table_name": "mmembstb",
            "description": "[가맹점 매장 마스터] 개별 매장(MS_NO) 기준으로 테스트 결제 상세 트랜잭션 정보를 분할 격리하여 연계합니다."
        },
        {
            "table_name": "mmslogtb",
            "description": "[웹 변경 이력 로그] 연동 테스트 모듈에서 결제 전송 테스트를 기동하거나 데이터 삭제/초기화 시 조작 감사 이력을 MMSLOGTB에 실시간 적재합니다."
        }
    ]
    ...
    try:
        ...
        print("Successfully updated table_memos.json with njs_inter_test_use details!")
