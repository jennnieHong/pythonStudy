...
def update_memos():
    ...
    if "njs_inter_test_goods" not in tables:
        print("njs_inter_test_goods key not found in tables. Initializing...")
        tables["njs_inter_test_goods"] = {
            "memo": "NJS_INTER_TEST_GOODS 테이블 정보입니다. (포스 연동 인터페이스 테스트 상품 상세)",
            "starred": False,
            "color": "none",
            "columns": {},
    ...
    njs_inter_test_goods = tables["njs_inter_test_goods"]
    
    # Add/Update related_tables with all verified related tables in the telex/interface testing flow
    njs_inter_test_goods["related_tables"] = [
        {
            "table_name": "njs_inter_test",
            "description": "[인터페이스 테스트 마스터] NJS_INTER_TEST_GOODS.TR_NO = NJS_INTER_TEST.TR_NO 조인을 통해 테스트 거래 마스터 전표 정보와 해당 전표의 상세 판매 품목 목록을 N:1 관계로 대조합니다."
        },
        {
            "table_name": "mgoodstb",
            "description": "[매장별 상품 마스터] NJS_INTER_TEST_GOODS.GOODS_CD = MGOODSTB.GOODS_CD 조인을 통해 테스트 판매 대상이 되는 개별 메뉴/단품의 표준 사양 및 포스 판매 단가 정보를 매핑합니다."
        },
        {
            "table_name": "tgoodstb",
        ...
        {
            "table_name": "mmembstb",
            "description": "[가맹점 매장 마스터] 개별 매장(MS_NO) 기준으로 테스트 상품 상세 트랜잭션 정보를 분할 격리하여 연계합니다."
        },
        {
            "table_name": "mmslogtb",
            "description": "[웹 변경 이력 로그] 연동 테스트 모듈에서 상품별 테스트 거래를 전송하거나 데이터 삭제/초기화 시 조작 감사 이력을 MMSLOGTB에 실시간 적재합니다."
        }
    ]
    ...
    try:
        ...
        print("Successfully updated table_memos.json with njs_inter_test_goods details!")
