...
| No | 케이스 | 세션 조건 | RequestBody | 예상값 |
|----|--------|----------|-------------|-------|
| 5-1 | 단건 정상 삭제 | chainNo=`C001`, A001 미사용 | `{"delAcntCdArr":["A001"],"acntFg":"0"}` | `{"fail":0,"succ":1}`, DB에서 A001 삭제 확인 |
...
| 5-11 | **다건 삭제 (3건)** | `["A001","A002","A003"]` 모두 미사용 | succ=3이지만 A001은 3회, A002는 2회 DELETE 시도 ★ |
| 5-12 | 단건 삭제 | `["A001"]` | list=1건, deleteCode 1회 → 정상 (버그 미발생) |
...
codeDel (deleteCode) — 건별 독립 처리
├── useCdChk > 0 → fail++ (사용중, 삭제 안 함)
└── useCdChk = 0 → deleteCode(list) → succ++
반환: {"fail": N, "succ": M}
```
...
$base = "http://localhost:18080/backoffice/data/hq/cash/hq_cash_00001"
...
Invoke-RestMethod -Uri "$base/codeSave" -Method POST `
  -Body '{"acntFg":"0","acntCd":"TEST01","acntNm":"테스트입금계정","statusFg":"Y"}' `
...
Invoke-RestMethod -Uri "$base/codeUpdate" -Method POST `
  -Body '{"acntFg":"0","acntCd":"TEST01","acntNm":"테스트입금계정(수정)"}' `
  -Headers $h -WebSession $session
# 예상: 1
