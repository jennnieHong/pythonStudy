# St_Emp_00002 — 비밀번호 관리 단위 테스트케이스 및 수행 결과
...
| 세션 키 | 값 예시 | 설명 |
|---------|---------|------|
| `msNo` | `NC0007` (카페 매니저 소속) | 로그인 사용자 매장번호 (user_ms_no) |
| `ID` | `fnbcafe` | 로그인 사용자 ID |
...
> - **E2E 검증 시나리오**: `fnbcafe` 계정으로 로그인 성공 ➔ `st_emp_00002` 화면 진입 ➔ 기존 비밀번호 `0000` 입력 ➔ 신규 비밀번호 `0000a123!` 입력 및 확인 ➔ 저장 클릭 ➔ 알림 다이얼로그(`비밀번호가 변경되었습니다.`) 성공 감지 ➔ 실제 DB 해시값 및 메타데이터(`FST_LOGIN_PW_CHANGE`='N') 수정 여부 확인 완료.
...
- **서비스 로직**: 세션 `msNo`(`NC0007`) + `ID`(`fnbcafe`) → `St_Emp_00002_Service.getuserInfo()` 호출
- **결과 데이터**: `MMEMBSTB.MS_NM` (카페), `MUSERSTB.USER_ID` (`fnbcafe`) 반환

| No | 테스트 조건 | 예상 결과 | **수행 결과 (DB 교차 확인)** | 판정 |
|----|----------|---------|-------------------------|------|
| 1-1 | 로그인 세션 유효 (msNo='NC0007', ID='fnbcafe') | 사원정보 DTO 정상 반환 | 개발 DB 상에 `fnbcafe`(매장코드 `NC0007`, 사용자명 `카페 매니저`) 데이터 존재 확인 완료 | **PASS** |
...
| 2-7 | `userPW` 파라미터 누락 | `upassWd=NewPass1!` | 400 (Bad Request) | `@RequestParam(required=true)`에 의한 필수값 제어 확인 | **PASS** |
| 2-8 | `upassWd` 파라미터 누락 | `userPW=0000` | 400 (Bad Request) | `@RequestParam(required=true)`에 의한 필수값 제어 확인 | **PASS** |

> ⚠️ **ST 화면 컨트롤러 유효성 검사 누락 결함 식별**:
> - `St_Emp_00002_Controller.modifiyCode` 에는 HQ 컨트롤러와 달리 `@NotBlank @Size(min=1, max=4000)` 어노테이션이 존재하지 않습니다.
> - 따라서 파라미터 값이 빈 문자열(`""`)로 입력될 때, 컨트롤러 레벨에서 걸러지지 않고 서비스로 즉시 진입되는 결함이 존재합니다. 
...
- **Oracle 전용 함수 잔존**:
  - `St_Emp_00002_Sql.xml` 의 `updateUsersTb` 쿼리 내 `SYSDATE` 문법 사용.
