...
├── [테이블] MGOODSTB (CUD 작업)
│   ├── (Trigger) MGOODS_T01 [AFTER EACH ROW : INSERT OR UPDATE OR DELETE]
│   ├── (Trigger) MGOODS_T02 [BEFORE EACH ROW : DELETE]
│   └── (Trigger) MGOODS_T03 [AFTER EACH ROW : INSERT OR UPDATE OR DELETE]
├── [테이블] MMEMBSTB (CUD 작업)
│   └── (Trigger) MMEMBS_T01 [AFTER EACH ROW : INSERT OR UPDATE OR DELETE]
├── [테이블] STRNCDTB (CUD 작업)
│   └── (Trigger) STRNCD_T01 [AFTER EACH ROW : INSERT]
└── [테이블] TGOODSTB (CUD 작업)
    └── (Trigger) TGOODS_T01 [AFTER EACH ROW : INSERT OR UPDATE OR DELETE]
```
